...
def train_simple_model(config, encoder, dropout_layer, classifier, training_data, epochs, map_relid2tempid, seen_des_by_id):
...
if __name__ == '__main__':
    ...
    if config.task == "FewRel":
        ...
        config.rel_des_file = "data/fewrel/relation_description_detail.txt"
        ...
        config.rel_des_file = "data/tacred/relation_description_detail.txt"
    ...
    for rou in range(config.total_round):
        ...
        relation_standard = {}
        forward_accs = []
        seen_des = {} # relation -> tokenized description
        
        for steps, (training_data, valid_data, test_data, current_relations, historic_test_data, seen_relations, seen_descriptions) in enumerate(sampler):
            ...
            seen_relid = []
            for rel in seen_relations:
                seen_relid.append(sampler.rel2id[rel])
            ...
            for data in history_data:
                ...
                accuracy.append(evaluate_model_rrf(config, encoder, dropout_layer, classifier, data, seen_relations, map_relid2tempid, rep_des, seen_relid))
            print(accuracy)
